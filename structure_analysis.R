...
### PLOT STRUCTURE
plot_structure_on_protein(prot_cons, structure_profiles = profiles_for_structure,pdb_name,structure_names = c("T1", "T2", "T3"), legend_pos = "topleft")
#######TUNNELS ENTROPY
...
p=2
plot(structure$stucture_probabilities[[p]]$stucture_probabilities,nm)
smoothScatter(structure$stucture_probabilities[[p]]$stucture_probabilities,profiles_for_structure$tunel_2$entropy[2,],ylim=c(0,1), pch=20 )
#KOLMOGOROV-SMIRNOF TEST FOR QUALITY OF DISTRIBURIONS
EQUAL=smirnof_kolmogorov_test(prot_cons, profiles_for_structure,alternative = 1, pdb_name,T)
